...
/**
 * One entry per title. Generic so the items keep the shape they arrived with —
 * untyped, this returned `any[]`, and every field read off a recommendation
 * downstream became an implicit `any`.
 *
...
/**
 * The improvement plan a prospect's audit implies.
 *
 * Annotated because the empty defaults alone infer `never[]`, which made every
 * caller passing real findings a type error and every item read off the result
 * an implicit `any`.
 *
 * @param {Record<string, any>} lead
 * @param {Array<Record<string, any>>} [websiteFindings]
 * @param {{ reviewed: boolean, score: number, findings: Array<Record<string, any>> }} [googleAudit]
 */
