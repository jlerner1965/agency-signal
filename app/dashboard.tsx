...
export default function Dashboard({ ownerName }: { ownerName: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/leads/${selectedId}`).then((response) => response.json()).then((payload) => setActivities(payload.activities ?? [])).catch(() => setActivities([]));
  }, [selectedId]);
  ...
  function chooseLead(lead: Lead) { setSelectedId(lead.id); setNotes(lead.notes ?? ""); setFollowUp(inputDate(lead.nextFollowUpAt)); setContactDraft({ contactName: lead.contactName, email: lead.email, phone: lead.phone, carrier: lead.carrier }); }
  ...
  async function patchLead(values: Record<string, unknown>, success: string) {
    ...
    try {
      ...
      const activityResponse = await fetch(`/api/leads/${selected.id}`); setActivities((await activityResponse.json()).activities ?? []);
  ...
  function outreach() {
    ...
    const weakest = [["visibility", selected.visibilityScore], ["website conversion", selected.conversionScore], ["technical experience", selected.technicalScore], ["visitor trust", selected.trustScore]].sort((a, b) => Number(a[1]) - Number(b[1]))[0][0];
    const firstName = selected.contactName.split(" ")[0] || "there";
    return {
      subject: `${selected.agencyName}: a few website opportunities`,
      body: `Hi ${firstName},\n\nI reviewed ${selected.agencyName}'s public website and found a few specific opportunities that could help turn more visitors into inquiries. The clearest gap is around ${weakest}.\n\nI put the evidence and recommended fixes into a short, private brief:\n${window.location.origin}/report/${selected.reportToken}\n\nWould a 15-minute walkthrough be useful?\n\nBest,\nJames Lerner`,
    };
  }
      ...
      <section className="workspace">
        ...
        <div className="workspace-content">
          ...
          <section className="pipeline-card">
            ...
            <div className="table-wrap"><table><thead><tr><th>Business</th><th>Industry</th><th>Audit score</th><th>Stage</th><th>Signal</th><th>Next action</th><th><span className="sr-only">Open</span></th></tr></thead><tbody>{filtered.map((lead) => (
              <tr key={lead.id} className={lead.id === selectedId ? "selected" : ""} onClick={() => chooseLead(lead)}>
                <td><div className="agency-cell"><span className="agency-avatar">{initials(lead.agencyName)}</span><span><strong>{lead.agencyName}</strong><small>{lead.contactName || "No contact"}{lead.city ? ` · ${lead.city}${lead.state ? `, ${lead.state}` : ""}` : ""}</small></span></div></td><td><span className="carrier-text">{lead.carrier}</span></td><td>{lead.score ? <span className={`score-badge ${scoreTone(lead.score)}`}><i style={{ "--score": `${lead.score * 3.6}deg` } as React.CSSProperties} />{lead.score}</span> : <span className="not-run">Not run</span>}</td><td><span className={`stage stage-${lead.status.toLowerCase().replaceAll(" ", "-")}`}>{lead.status}</span></td><td>{lead.reportViews ? <span className="signal"><i /> {lead.reportViews} view{lead.reportViews === 1 ? "" : "s"}</span> : <span className="muted">No activity</span>}</td><td><span className={lead.status === "Follow-up due" ? "due" : "next-action"}>{lead.nextFollowUpAt ? friendlyDate(lead.nextFollowUpAt) : !lead.score ? "Run audit" : lead.status === "Audit ready" ? "Send brief" : "Add follow-up"}</span></td><td><button className="row-open" aria-label={`Open ${lead.agencyName}`} onClick={(event) => { event.stopPropagation(); chooseLead(lead); }}>›</button></td>
