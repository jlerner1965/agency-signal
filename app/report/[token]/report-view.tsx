...
type ReportPayload = { lead: PublicReportLead; findings: Finding[]; opportunity: Opportunity; audit: { pagesAudited: number; confidenceScore: number; checksPassed: number; checksFailed: number; checksUnverified: number; createdAt: string } | null };
...
export default function ReportView({ token }: { token: string }) {
  ...
  const { lead, findings, opportunity, audit } = payload;
