...
/**
 * What a mockup reads its brand from, taken out of the crawl payload in the
 * shape it is actually stored in.
 *
 * That shape is not the crawler's. The collectors run every page through
 * `distillPage` before storing it, which replaces the markup with parsed
 * fields and keeps the homepage's own markup under `rawHead` — and only the
 * homepage's, because `keepMarkup` is set for the first page alone. Reading a
 * page here for `html`, the field the crawler uses before distillation, finds
 * nothing and silently costs the logo: a mockup without one is a supported
 * outcome, so nothing fails.
 *
 * Both halves matter and for different things. The palette usually lives in the
 * stylesheets; the logo only ever lives in the markup.
 *
 * @param {{ pages?: Array<Record<string, any>>, homeCss?: string } | null | undefined} payload
 *   one stored crawl payload, pages already distilled
 * @returns {{ source: string, url: string }} the text to read, and the page it
 *   came from — a logo is usually a relative path and needs that to resolve.
 */
export function brandSourceFromCrawl(payload) {
  const pages = Array.isArray(payload?.pages) ? payload.pages : [];
  // The page whose markup was kept. Falling back to the first page keeps the
  // base URL right when the homepage could not be read and no markup was kept.
  const home = pages.find((page) => page?.ok && page?.rawHead) ?? pages[0];
  return {
    source: `${String(home?.rawHead ?? "")}\n${String(payload?.homeCss ?? "")}`,
    url: String(home?.url ?? ""),
  };
}
