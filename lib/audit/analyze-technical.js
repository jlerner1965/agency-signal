...
/**
 * Whether a redirect only reached the canonical form of the same address.
 *
 * An audit target is normalised to https before it is fetched, so a site that
 * enforces www or upgrades the scheme redirects by definition — that is correct
 * configuration, not a defect, and calling it one would flag almost every
 * well-run site. A redirect that changes the host or lands on a different path
 * is a different claim and worth making. With nothing to compare against, no
 * defect is asserted.
 */
export function canonicalRedirect(requested, final) {
  ...
  try {
    ...
    return host(from) === host(to) && path(from) === path(to);
  } catch {
    return true;
  }
}
...
export function analyzeTechnical(payloads) {
  ...
  const { status, requestedUrl = "", finalUrl = "", html = "", redirected = false } = document.payload ?? {};
  ...
  // A redirect is a measured fact, so this check has an answer either way.
  // Reporting it as unverified said "the homepage redirects" and "we could not
  // tell" in the same line, and quietly cost the run coverage for a site that
  // is configured correctly.
  const canonical = !redirected || canonicalRedirect(requestedUrl, finalUrl);
  check("redirects", "Technical", "Direct homepage response", 2, canonical,
    !redirected
      ? "The homepage responded directly with no redirect."
      : canonical
        ? `The homepage redirected to its canonical address at ${finalUrl}, which is how a single hostname is enforced.`
        : `The homepage at ${requestedUrl} redirected to ${finalUrl}.`,
    { severity: "Low", title: "The homepage is not served where it is advertised",
      recommendation: `Serve the homepage directly at ${requestedUrl || "the advertised address"}, or publish ${finalUrl} as the address everywhere the business is listed.`,
      impactNote: "Every hop is a round trip before anything renders, and an address that redirects elsewhere splits its links and its ranking signals between two URLs.",
