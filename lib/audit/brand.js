...
export function extractFontStack(html) {
  const bodyRule = html.match(/(?:^|[{;\s])body\s*\{[^}]*font-family\s*:\s*([^;}]+)/i)?.[1]
    ...
    ?? "";
  const stack = bodyRule.replace(/!important/i, "").replace(/["']/g, "").split(",").map((font) => font.trim()).filter(Boolean);
  const googleFonts = [...html.matchAll(/fonts\.googleapis\.com\/css2?\?family=([^"'&]+)/gi)]
    .map((match) => decodeURIComponent(match[1]).split(":")[0].replace(/\+/g, " "));
  return { stack: stack.slice(0, 4), googleFonts: [...new Set(googleFonts)].slice(0, 3) };
}
