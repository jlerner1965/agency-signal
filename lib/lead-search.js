...
function actionRank(lead, now) {
  if (["Won", "Lost"].includes(lead.status)) return 99;
  if (lead.nextFollowUpAt && new Date(lead.nextFollowUpAt).getTime() <= now.getTime()) return 0;
  if (lead.reportViews > 0 && lead.status !== "Meeting booked") return 1;
  if (lead.status === "Audit ready") return 2;
...
export function nextLeadAction(lead, now = new Date()) {
  ...
  if (rank === 2) return lead.email ? "Send opportunity brief" : "Add decision-maker email";
  if (rank === 3) return "Run website audit";
