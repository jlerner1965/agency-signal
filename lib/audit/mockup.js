...
.brand .mark{width:34px;height:34px;flex:none;background-size:contain;background-repeat:no-repeat;background-position:center}
...
<body>
<div class="stamp">CONCEPT MOCKUP — ${escapeHtml(stampSource(brand))} Not a live website.</div>
${body}
</body></html>`;
}

/**
 * A logo URL safe to sit inside a CSS `url()`, or nothing.
 *
 * The value was read out of the prospect's own markup, and the concept page is
 * served to a sandbox with no scripts — but a URL carrying a quote or a paren
 * would close the declaration rather than be drawn, so it is refused outright
 * instead of escaped. A concept without a logo is a supported outcome.
 */
...
/**
 * What this page was built from, as far as it is true.
 *
 * Every brand token has a documented fallback, and `defaulted` records which
 * ones were used — so a site whose palette or font stack could not be read
 * produced a page in stock colours under a stamp claiming they were the
 * prospect's own.
 */
function stampSource(brand) {
  const name = brand.businessName || "this business";
  const defaulted = Array.isArray(brand.defaulted) ? brand.defaulted : [];
  const readColours = !defaulted.includes("primary");
  const readType = Boolean(brand.fontStack) && !/^Georgia,/.test(String(brand.fontStack));
  // Colours and type are in the page itself, so claiming them is checkable by
  // looking. The logo is a URL read from their markup that may or may not still
  // resolve, and a stamp naming a logo the reader cannot see is worse than one
  // that does not mention it.
  const read = [readColours && "colours", readType && "type"].filter(Boolean);
  return read.length
    ? `built from ${name}'s own ${read.join(", ").replace(/, ([^,]*)$/, " and $1")}.`
    : `a layout for ${name}, built from what the audit could read of their site.`;
}

function header(brand, services, words) {
  // Drawn as a background, not an <img>. A logo URL that 404s — and a guessed
  // one often does — rendered the broken-image icon and its alt text beside the
  // wordmark, so the prospect's own name appeared twice on a page carrying
  // their brand, once as a broken image. A background that fails draws nothing.
  const mark = cssUrl(brand.logoUrl);
  const logo = mark
    ? `<span class="mark" style="background-image:url(${mark})" aria-hidden="true"></span>`
    : "";
