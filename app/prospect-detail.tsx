...
export default function ProspectDetail(props: Props) {
  ...
  const [proposalTitle, setProposalTitle] = useState(recommendedOffer.name);
  const [proposalOutcome, setProposalOutcome] = useState(recommendedOffer.outcome);
  const [proposalScope, setProposalScope] = useState(opportunity?.primaryFinding || lead.painPoint || "Address the highest-priority opportunity identified in the website audit.");
  const [proposalDeliverables, setProposalDeliverables] = useState(recommendedOffer.deliverables.join("\n"));
  ...
  if (draftLeadId !== lead.id) {
    ...
    setProposalTitle(recommendedOffer.name); setProposalOutcome(recommendedOffer.outcome); setProposalScope(opportunity?.primaryFinding || lead.painPoint || "Address the highest-priority opportunity identified in the website audit."); setProposalDeliverables(recommendedOffer.deliverables.join("\n"));
  }
  ...
  async function generateProposal() {
    const deliverables = proposalDeliverables.split("\n").map((item) => item.trim()).filter(Boolean);
    const response = await fetch(`/api/leads/${lead.id}/proposal`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ offerId, price: proposalPrice, timeline: proposalTimeline, title: proposalTitle, outcome: proposalOutcome, scope: proposalScope, deliverables }) });
  ...
  return <aside className="detail-panel conversion-panel">
    ...
    {tab === "close" && <>
      ...
      <section className="conversion-section proposal-builder"><div className="conversion-title"><div><span>06</span><h3>Proposal</h3></div><small>Edit before sharing</small></div>{proposal ? <div className="proposal-status"><div><span className={`proposal-state-pill ${proposal.status.toLowerCase()}`}>{proposal.status}</span><strong>{proposal.title}</strong><p>${proposal.price.toLocaleString("en-US")} · {proposal.timeline} · {proposal.viewCount} view{proposal.viewCount === 1 ? "" : "s"}</p></div><div className="dual-actions"><button onClick={copyProposal}>Copy proposal link</button><a href={`/proposal/${proposal.token}`} target="_blank" rel="noreferrer">Open / save PDF ↗</a></div><button className="refresh-proposal" onClick={props.onRefresh}>Refresh views and acceptance</button></div> : <><label>Starting offer<select value={offerId} onChange={(event) => { const next = offerCatalog.find((item) => item.id === event.target.value)!; setOfferId(next.id); setProposalPrice(next.price); setProposalTimeline(next.timeline); setProposalTitle(next.name); setProposalOutcome(next.outcome); setProposalDeliverables(next.deliverables.join("\n")); }}>{offerCatalog.map((offer) => <option key={offer.id} value={offer.id}>{offer.name}</option>)}</select></label><div className="proposal-edit-grid"><label>Proposal title<input value={proposalTitle} onChange={(event) => setProposalTitle(event.target.value)} /></label><label>Desired outcome<textarea rows={3} value={proposalOutcome} onChange={(event) => setProposalOutcome(event.target.value)} /></label><label>Why this work<textarea rows={3} value={proposalScope} onChange={(event) => setProposalScope(event.target.value)} /></label><label>Deliverables <small>One per line</small><textarea rows={5} value={proposalDeliverables} onChange={(event) => setProposalDeliverables(event.target.value)} /></label></div><div className="proposal-inputs"><label>Investment<input type="number" min="500" step="100" value={proposalPrice} onChange={(event) => setProposalPrice(Number(event.target.value))} /></label><label>Timeline<input value={proposalTimeline} onChange={(event) => setProposalTimeline(event.target.value)} /></label></div><div className="offer-preview"><strong>Built from the audit evidence</strong><p>{selectedOffer.proof}</p></div><button className="generate-proposal" disabled={busy || !lead.score || !proposalTitle.trim() || !proposalDeliverables.trim()} onClick={() => generateProposal().catch((error) => alert(error.message))}>{lead.score ? "Create trackable proposal" : "Run audit before proposal"}</button></>}</section>
      </div></details>
      </>}
    </>}

    {tab === "audit" && <div className="audit-coverage"><span>Up to 5 pages</span><span>Mobile Lighthouse</span><span>SEO + accessibility</span></div>}
