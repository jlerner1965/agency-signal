...
export function analyzeGooglePresence(payloads) {
  ...
  // Whether a person has actually looked at the live profile and recorded what
  // it shows. That is a source, not a consolation prize: the Places API does
  // not return the services section or the activity figures for a profile we do
  // not own, so a hand-entered profile is the only way to measure those at all.
  const reviewed = Boolean(manual.reviewed);

  /**
   * The four fields no API returns. A person's entry feeds the same checks.
   * Shared by both paths, because whether the category came from the API or
   * from a person has no bearing on how the photo count is judged.
   */
  ...
  // Nobody has read the profile — not the API, and not a person. Nothing about
  // it is measured, and that is reported as missing data rather than guessed at.
  if (!profile && !reviewed) {
    ...
    findings.push({
      category: "Trust",
      severity: "Medium",
      title: "The Google Business Profile could not be assessed",
      evidence: "No Google Business Profile data was available for this audit, so nothing about the profile was measured. This is missing data rather than a weak profile.",
      // Written for whoever reads the document. Naming an environment variable
      // here put a line of this tool's own configuration into a proposal.
      recommendation: "Have the profile reviewed alongside the site — category, services, reviews and hours are the strongest local signals a business controls.",
  ...
  // Read by a person rather than by the API. The checks the entered fields can
  // answer are measured from them; the ones nobody was asked for stay
  // unverified, because an unasked question is not a passed check.
  if (!profile) {
    const category = String(manual.googlePrimaryCategory ?? "").trim();
    check("gbp-category", "Primary category is set", 5, Boolean(category),
      category ? `The profile's primary category is “${category}”, as recorded from the live profile.` : "No primary category was recorded from the profile.",
    ...
    check("gbp-nap", "Name, address and phone match the site", 5, Boolean(manual.googleNapConsistent),
      manual.googleNapConsistent
        ? "The name, address and phone on the profile were confirmed to match the site."
        : "The name, address and phone on the profile were not confirmed to match the site.",
    ...
    const reviewCount = Number(manual.reviewCount ?? 0);
    const rating = Number(manual.rating ?? 0);
    check("gbp-reviews", "Review volume", 4, reviewCount >= 25,
      reviewCount ? `The profile has ${reviewCount} reviews at ${rating || "no"} average rating, as recorded from the live profile.` : "No reviews were recorded on the profile.",
    ...
    // Not on the form, so nobody was asked. Left in scope and unverified rather
    // than omitted — an omitted check reads as a pass.
    for (const [id, label, weight] of [["gbp-hours", "Opening hours published", 3], ["gbp-status", "Profile is operational", 2]]) {
      checks.push({
        id, category: "Trust", label, weight, status: "unverified", earned: 0,
        evidence: `${label} is not among the fields recorded from the profile, so it was not measured.`,
        unverifiedReason: "not-applicable",
      });
    }
    ...
    return {
      findings, checks, reachable: true, manualChecks: [],
      message: `Read the Google profile from a manual review${category ? ` (${category})` : ""}; ${verified} of ${checks.length} checks verified.`,
    };
  }
