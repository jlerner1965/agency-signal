...
export const sectionCatalog = [
  ...
  },
  {
    ...
    detail: "Live concept pages built from the prospect's own site — pages, not pictures of pages.",
  },
  {
    id: "scope",
...
export function sectionOptions(inventory = {}) {
  ...
  const state = {
    ...
    concepts: {
      ...
      // What the builder guarantees. Every brand token has a documented
      // fallback, so a site whose palette, font stack or logo could not be read
      // still produces a page — and promising the prospect's own logo on it was
      // a claim this could not keep.
      note: built > 0
        ? `${plural(built, "concept page")} already built${buildable ? ", rebuilt on each build" : ""}.`
        : "Two live concept pages, built from whatever the audit could read on their site.",
