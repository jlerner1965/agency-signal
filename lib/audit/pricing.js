...
export function selectDeliverables(config, audit) {
  ...
  // The line cites the coverage findings as its justification, so it cannot be
  // offered when none of them is in play — a priced line citing nothing is the
  // one thing the evidence rule exists to prevent, and an operator choosing
  // which findings to raise can otherwise reach exactly that state.
  const orphans = serviceLines.filter((line) => line.hasLandingPage === false);
  if (orphans.length && coverage.length) {
